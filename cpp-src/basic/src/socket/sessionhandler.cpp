...
void basic::SessionHandler::start()
{
   ...
   this->sessionThread = std::make_shared<std::thread>([this]
                                                       {  run(); });
}
...
void basic::SessionHandler::process(const std::vector<std::string> &results, Session &session)
{
   ...
   for (auto s : results)
   {
      ...
      std::cerr << "Message Recieved: " <<s<< std::endl;
...
std::vector<std::string> basic::SessionHandler::splitter(Session &s, const char *raw, int len)
{
   ...
   // if (sDebug > 1)
   std::stringstream ss(std::string(raw, len)); // Initialize stringstream with the buffer content
   std::string item;
